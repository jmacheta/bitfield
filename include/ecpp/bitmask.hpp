...
template <std::unsigned_integral T> struct bitmask {
  ...
  constexpr auto &operator=(value_type v) noexcept { return m_value = v; }
...
using larger_of_t = typename std::conditional_t<sizeof(V) < sizeof(U), U, V>;
} // namespace bf_impl

// Bitwise operators with bitmasks
template <typename T, typename U>
[[nodiscard]] constexpr auto operator|(bitmask<T> const &a, bitmask<U> const &b) noexcept {
